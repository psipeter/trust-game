...
def plot_metrics(data_gen, data_score, learner_plays):
	...
	fig, axes = plt.subplots(nrows=1, ncols=len(metrics_gen), figsize=((len(metrics_gen)*1.5, 2)), sharey=True)
	axes[0].set(ylim=((0, 1)), yticks=((0, 1)))
	for i, metric in enumerate(metrics_gen):
		if metric in ['mean', 'gift', 'defect']:
			sns.histplot(data=data_gen, x=metric, bins=np.arange(0, 1.1, 0.1), stat='probability', ax=axes[i])
			axes[i].set(title=metric, xlim=((0, 1)), xticks=((0, 1)))
		elif metric in ['punish', 'forgive']:
			sns.histplot(data=data_gen, x=metric, bins=np.arange(-1.1, 1.1, 0.1), stat='probability', ax=axes[i])
			axes[i].set(title=metric, xlim=((-1, 1)), xticks=((-1, 1)))
		else:
			sns.histplot(data=data_gen, x=metric, bins=10, stat='probability', ax=axes[i])
			axes[i].set(title=metric)
	fig.savefig(f'plots/generosity_metrics_{learner_plays}.pdf')

	fig, axes = plt.subplots(nrows=1, ncols=len(metrics_score), figsize=((len(metrics_score)*1.5, 2)), sharey=True)
	axes[0].set(ylim=((0, 1)), yticks=((0, 1)))
